...
class TestViewModel: NSObject, ObservableObject {
    ...
    @Published var clipUrl: String? // DID clip ID
    
    @Published var fullTestResponses: [String] = []
    @Published var fullTestFeedbacks: [String] = []
...
//    @Published var audioPower: Double = 0.0
    ...
    @Published var testName: String = "" // for firestore
    @Published var oneTypeQuestionText: String = "" // for firestore
...
//    }
    ...
    // ***************************************************************
    func stopRecording() { // this is for one type test
        audioEngine.stop()
            ...
            DispatchQueue.main.async {
                ...
                case .success(let chatResult):
                    if let feedback = chatResult.choices.last?.message.content {
                        ...
                        self?.fullTestResponses.append(responseText)
                        self?.fullTestFeedbacks.append(feedback)
                    }
                case .failure(let error):
    ...
    func resetTestData() {
        fullTestResponses.removeAll()
        fullTestFeedbacks.removeAll()
    }
...
//    }
    ...
    func sendResponseForScoring(response: String) { // this is for one type test
    ...
    func sendResponseForGradingWithFeedback() { // this is for one type test
