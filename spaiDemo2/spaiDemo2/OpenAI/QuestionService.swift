...
struct QuestionService {
    static let openAI = OpenAI(apiToken: "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")
    ...
    static func fetchListeningComprehensionQuestion(completion: @escaping (String?) -> Void) {
        let query = ChatQuery(
            model: .gpt4_1106_preview,
            messages: [.init(role: .user, content: "Create a concise four-sentence paragraph for a CEFR B2 level listening comprehension exercise, focusing on a spoken narrative such as a story, book, movie, article, or everyday topics like work, education, nature, or technology. The paragraph should test the user's ability to understand and summarize spoken English. Begin with the statement: 'I will read a short paragraph twice. After I'm done, please provide a detailed summary of it.' Then, clearly indicate the start of the first reading with 'First reading:' followed by the content. After the first reading, signal the start of the second reading with 'Second reading:' and repeat the content. Conclude with 'Now, could you please summarize this paragraph?' Ensure the narrative is straightforward and excludes any introductory phrases.")],
            maxTokens: 300
        )
